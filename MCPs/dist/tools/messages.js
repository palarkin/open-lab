...
function messagingError(e) {
    const msg = e instanceof Error ? e.message : String(e);
    if (isMessagingGated(msg))
        return NOTE + `\n\n(underlying error: ${msg})`;
    return handleError(e);
}
export function registerMessageTools(server) {
    server.registerTool("ownerrez_list_thread_messages", {
        title: "List Thread Messages",
        description: "Fetch all messages on a single conversation thread. Requires a thread_id (OwnerRez has no " +
    ...
    server.registerTool("ownerrez_get_message", {
        title: "Get Message",
    ...
    server.registerTool("ownerrez_send_message", {
        title: "Send Message",
        description: "Post a message to a guest thread. NOTE: unavailable with a Personal Access Token (requires an OAuth app). " +
