...
export function registerBookingTools(server) {
    server.registerTool("ownerrez_list_bookings", {
        title: "List Bookings",
        description: "Search bookings. Requires property_ids OR since_utc. Compact view by default (id, property, guest, arrival, departure, status); full=true for every field. Use limit to cap. include_charges adds owner_amount/commission (implies full).",
        inputSchema: {
        ...
        try {
            ...
            const query = { ...rest, property_ids: property_ids?.join(","), include_guest: true };
            let items = await orPaged("/bookings", query);
            if (limit)
    ...
    server.registerTool("ownerrez_get_booking", {
        title: "Get Booking",
    ...
    server.registerTool("ownerrez_get_schedule", {
        ...
        try {
            ...
            const bookings = await orPaged("/bookings", { property_ids: ids.join(","), from, to, status: "active", include_guest: true });
