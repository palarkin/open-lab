...
export function registerPrompts(server) {
    server.registerPrompt("weekly_turnover_briefing", {
    ...
    }, ({ from, to }) => userMsg(`Use ownerrez_get_schedule for ${from} to ${to}. Give me a concise operational briefing grouped by day: ` +
    ...
    server.registerPrompt("monthly_owner_statement", {
    ...
    }, ({ owner, period }) => userMsg(`Find the owner matching "${owner}" with ownerrez_list_owners, then get their statement with ownerrez_get_owner_statement` +
    ...
    server.registerPrompt("unanswered_inquiries", {
    ...
    }, ({ since }) => userMsg(`Use ownerrez_list_inquiries with since_utc=${since}. For each recent lead, read its thread with ownerrez_list_thread_messages ` +
