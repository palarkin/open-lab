...
export function registerInquiryTools(server) {
    server.registerTool("ownerrez_list_inquiries", {
        title: "List Inquiries (leads)",
        description: "List pre-booking guest inquiries/leads: requested dates, party size, source channel, and the " +
        ...
        try {
            const query = { property_ids: property_ids?.join(","), since_utc, include_guest: true };
            let items = await orPaged("/inquiries", query);
    ...
    server.registerTool("ownerrez_get_inquiry", {
        title: "Get Inquiry",
