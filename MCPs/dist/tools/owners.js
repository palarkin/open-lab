...
async function listGenuineStatements(owner_id) {
    const all = await orLegacyGet("/ownerstatements", { ownerId: owner_id });
    return (Array.isArray(all) ? all : []).filter((s) => s.OwnerId === owner_id);
}
function shapeGenuine(s) {
    return {
...
async function synthesize(owner_id, property_ids, from, to, include_bookings) {
    const allProps = await orPaged("/properties");
    const targets = property_ids?.length ? allProps.filter((p) => property_ids.includes(p.id)) : allProps.filter((p) => p.owner_id === owner_id);
    if (!targets.length)
        return { source: "synthesized", warning: "No matching properties for this owner/ids.", owner_id, property_ids };
    const bookings = await orPaged("/bookings", { property_ids: targets.map((p) => p.id).join(","), from, to, include_charges: true });
...
export function registerOwnerTools(server) {
    server.registerTool("ownerrez_list_owners", { title: "List Owners", description: "List all property owners (id, name, contact, last_owner_statement_id).", inputSchema: { active: z.boolean().optional(), include_fields: z.boolean().optional() }, annotations: READ }, async (params) => {
        try {
            const items = await orPaged("/owners", params);
    ...
    server.registerTool("ownerrez_get_owner", { title: "Get Owner", description: "Fetch a single owner by id.", inputSchema: { id: z.number().int() }, annotations: READ }, async ({ id }) => {
        try {
            const data = await orRequest("get", `/owners/${id}`);
    ...
    server.registerTool("ownerrez_list_owner_statements", {
    ...
    server.registerTool("ownerrez_get_owner_statement", {
        title: "Get Owner Statement",
        description: "Return an owner statement. Prefers OwnerRez's official statement (by statement_id, or the latest / period-matching one for an owner_id); if none exists or the official source is unavailable, synthesizes one from booking charges (needs from+to). Set synthesize=true to force the estimate.",
        inputSchema: {
        ...
        try {
            if (!force) {
                ...
                if (statement_id) {
                    const s = await orLegacyGet(`/ownerstatements/${statement_id}`);
                ...
                if (owner_id) {
                    ...
                    if (stmts.length) {
                        let pick;
                        if (from && to)
                            pick = stmts.filter((s) => day(s.w_StatementFromDate) <= to && day(s.StatementDate) >= from).sort((a, b) => day(b.StatementDate).localeCompare(day(a.StatementDate)))[0];
                        if (!pick)
                            pick = stmts.sort((a, b) => day(b.StatementDate).localeCompare(day(a.StatementDate)))[0];
                        if (pick)
                            return { content: [{ type: "text", text: formatResponse(shapeGenuine(pick)) }] };
                    }
            ...
            if (canSynth) {
                ...
                if (!force)
                    out.note = "No official statement matched — " + out.note;
            ...
            if (!force && canSynth) {
                try {
                    const out = await synthesize(owner_id, property_ids, from, to, include_bookings);
                    out.note = `Official statement unavailable (${e instanceof Error ? e.message : e}); showing synthesized estimate. ` + out.note;
                    return { content: [{ type: "text", text: formatResponse(out) }] };
                }
                catch (e2) {
                    return { content: [{ type: "text", text: handleError(e2) }] };
                }
            }
            return { content: [{ type: "text", text: handleError(e) }] };
        }
