...
import { z } from "zod";
export const round = (n) => Math.round((n ?? 0) * 100) / 100;
export const day = (s) => (s ?? "").slice(0, 10);
...
export function normalizeNextUrl(next) {
...
export function aggregateStatement(targets, bookings, includeBookings = false) {
    const per = new Map();
    for (const p of targets)
        per.set(p.id, { property_id: p.id, property: p.name, bookings: 0, rent: 0, owner_payout: 0, commission: 0, total_charges: 0 });
    const detail = [];
    for (const b of bookings) {
        if (b.is_block || b.status === "canceled")
            continue;
        ...
        agg.rent += bRent;
        agg.owner_payout += bOwner;
        agg.commission += bComm;
        agg.total_charges += bTotal;
        if (includeBookings)
            detail.push({ booking_id: b.id, property_id: b.property_id, arrival: day(b.arrival), departure: day(b.departure), rent: round(bRent), owner_payout: round(bOwner), commission: round(bComm) });
    }
    ...
    const totals = properties.reduce((t, a) => ({ bookings: t.bookings + a.bookings, rent: round(t.rent + a.rent), owner_payout: round(t.owner_payout + a.owner_payout), commission: round(t.commission + a.commission), total_charges: round(t.total_charges + a.total_charges) }), { bookings: 0, rent: 0, owner_payout: 0, commission: 0, total_charges: 0 });
...
export function bucketSchedule(rows, from, to) {
...
export function isMessagingGated(errMsg) {
