...
//
// Usage:  node oauth-setup.mjs <client_id> <client_secret> <tc_code>
//
// Get <tc_code>: open the authorize URL in your browser, approve, and copy the value after
// "code=" from the address bar (the staygather.com page itself doesn't need to load).
import { readFileSync, writeFileSync } from "node:fs";

const REDIRECT = "https://staygather.com/callback/"; // must match the app's OAuth Redirect URL exactly
const [clientId, clientSecret, code] = process.argv.slice(2);
if (!clientId || !clientSecret || !code) {
  console.error("Usage: node oauth-setup.mjs <client_id> <client_secret> <tc_code>");
  process.exit(1);
}
