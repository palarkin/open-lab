...
function authOpts(preferBasic = false) {
    ...
    if (!chosen)
        throw new Error("Set OWNERREZ_OAUTH_TOKEN, or OWNERREZ_EMAIL + OWNERREZ_TOKEN.");
    return chosen;
}
...
let _props = null;
let _propsAt = 0;
export async function getPropertiesCached(ttlMs = 300000) {
    if (!_props || Date.now() - _propsAt > ttlMs) {
        _props = await orPaged("/properties", { active: true });
        _propsAt = Date.now();
    }
...
export async function orLegacyGet(path, params) {
    const res = await legacyClient().request({ method: "get", url: path, params });
    return res.data;
}
export async function orRequest(method, path, options = {}) {
    let attempt = 0;
    while (true) {
        try {
            ...
            if (e.request)
                throw new Error(`OwnerRez network error (${path}): no response`);
            throw err;
        }
    }
}
...
export async function orPaged(path, params = {}) {
    const out = [];
    let page = await orRequest("get", path, { params });
    out.push(...(page.items ?? []));
    while (page.next_page_url) {
        page = await orRequest("get", normalizeNextUrl(page.next_page_url));
        out.push(...(page.items ?? []));
    }
    return out;
}
export const formatResponse = (d) => JSON.stringify(d, null, 2);
export const handleError = (e) => `Error: ${e instanceof Error ? e.message : String(e)}`;
